...
#         # Connexion réussie
#         session['user_id'] = str(account.userID)
#         session['kidIndex'] = "0"  # tu peux le rendre dynamique selon ton app

#         # Passer les variables dynamiques au template
#         return render_template(
#             "users/home.html",
#             parent=account,
#             userID=account.userID,  # ou autre champ
#             kidIndex="0"  # ou dynamique si tu as plusieurs enfants
#         )
#     return render_template("users/login.html")
...
def login_sms():
    if request.method == 'POST':
        ...
        # Sauvegarder user_id dans la session pour la vérification
        session['user_id'] = user_id
        return redirect(url_for('auth.verify'))  # <-- ici il faut return
...
def verify():
    if request.method == 'POST':
        entered_code = request.form.get('otp')  # <-- ici 'otp' et pas 'code'
        correct_code = "123456"
        user_id = session.get('user_id')
